...
if (is_post()) {
    ...
    try {
        $existing = $fundingAccounts->getActiveAccountForUser((int) $user['id']);
        $account = $fundingAccounts->ensureActiveAccountForUser(
            (int) $user['id'],
...
$displayAccounts = $fundingAccounts->displayAccountsForUser((int) $user['id']);
$account = $fundingAccounts->primaryDisplayAccountForUser((int) $user['id']);
$multiProviderFunding = $fundingAccounts->multiProviderFunding();
...
$assignedDisplayAccounts = array_values(array_filter($displayAccounts, static function (array $row): bool {
    ...
    <div class="grid grid-cols-1 xl:grid-cols-5 gap-4 stagger-2">
        ...
        <section class="xl:col-span-3 user-premium-card bg-white rounded-2xl shadow-card border border-gem-border p-5">
            <div class="flex items-start justify-between gap-4 mb-4">
                <div>
                    <h2 class="text-[16px] font-bold text-gem-text"><?= $multiProviderFunding ? 'Your Funding Accounts' : 'Your Funding Account'; ?></h2>
                    <p class="text-[13px] text-gem-muted mt-0.5">Transfer from any Nigerian bank to this account.</p>
                </div>
                <?php if ($assignedDisplayAccounts !== []): ?>
            ...
            <?php if ($assignedDisplayAccounts !== []): ?>
                <div class="space-y-4">
                    <?php foreach ($assignedDisplayAccounts as $displayAccount): ?>
                        <?php
                        $displayBankName = (string) ($displayAccount['bank_name'] ?? '');
                        $displayAccountNumber = (string) ($displayAccount['dedicated_account_number'] ?? '');
                        $displayAccountName = (string) ($displayAccount['account_name'] ?? '');
                        $displayFullDetails = trim($displayBankName . "\n" . $displayAccountNumber . "\n" . $displayAccountName);
                        ?>
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                            <div class="user-premium-card rounded-2xl bg-gem-gray border border-gem-border p-4">
                                ...
                                <div class="text-[15px] font-bold text-gem-text mt-1"><?= e($displayBankName); ?></div>
                            </div>
                            <div class="user-premium-card rounded-2xl bg-gem-gray border border-gem-border p-4">
                                <div class="text-[11px] text-gem-muted font-bold uppercase tracking-wider">Account Number</div>
                                <div class="text-[15px] font-bold text-gem-text font-mono mt-1"><?= e($displayAccountNumber); ?></div>
                            </div>
                            <div class="user-premium-card rounded-2xl bg-gem-gray border border-gem-border p-4">
                                <div class="text-[11px] text-gem-muted font-bold uppercase tracking-wider">Account Name</div>
                                <div class="text-[15px] font-bold text-gem-text mt-1"><?= e($displayAccountName); ?></div>
                            </div>
                        </div>
                        <div class="flex flex-wrap gap-2">
                            <button class="inline-flex items-center gap-2 bg-gem-blue hover:bg-gem-blueDk text-white text-[13px] font-bold px-4 py-2.5 rounded-xl shadow-panel" type="button" data-copy-value="<?= e($displayAccountNumber); ?>">
                            ...
                            <button class="inline-flex items-center gap-2 border border-gem-border text-gem-text text-[13px] font-bold px-4 py-2.5 rounded-xl hover:bg-gem-gray" type="button" data-copy-value="<?= e($displayFullDetails); ?>">
                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="11" height="11" rx="2"/><path stroke-linecap="round" stroke-linejoin="round" d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/></svg>
                                Copy Full Details
                            </button>
                        </div>
            ...
            <?php else: ?>
                <div class="user-empty-state rounded-2xl bg-gem-gray border border-gem-border p-4 text-[13px] text-gem-muted">
                    <?= $status === 'failed' ? 'We could not generate your funding account. Please contact support or try again.' : 'Generate a funding account to receive bank transfer details.'; ?>
                </div>
